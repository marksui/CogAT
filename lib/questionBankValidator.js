...
function getSeriesCandidates(terms) {
  ...
  for (let cycleLength = 3; cycleLength <= Math.min(5, Math.floor(differences.length / 2)); cycleLength += 1) {
    const valid = differences.every((difference, index) => nearlyEqual(difference, differences[index % cycleLength]));
    const varied = differences.slice(0, cycleLength).some((difference) => !nearlyEqual(difference, differences[0]));
    add(
      `repeating ${cycleLength}-step differences`,
      terms.at(-1) + differences[differences.length % cycleLength],
      valid && varied,
    );
  }

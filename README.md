...
The app stores question results, the fixed 50-question daily practice, streaks, and unfinished daily sessions in the browser's `localStorage`. No account or server is required. Use **Progress backup → Export JSON** and **Import JSON** to move that history between browsers or devices.
